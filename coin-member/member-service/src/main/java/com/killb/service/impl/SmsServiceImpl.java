...
import com.killb.mapper.SmsMapper;
...
public class SmsServiceImpl extends ServiceImpl<SmsMapper, Sms> implements SmsService {
    ...
    private SendSmsRequest buildSmsRequest(Sms sms) {
        ...
        Config configByCode = configService.getConfigByCode("SMS_PHONE");
        if (configByCode == null) {
            throw new IllegalArgumentException("您输入的签名不存在");
        }
        sendSmsRequest.setSignName(configByCode.getName()); // 设置签名
        ...
        redisTemplate.opsForValue().set("SMS:" + sms.getTemplateCode() + ":" + sms.getMobile(), code);// -1, TimeUnit.MINUTES
        ...
        String content = configByCode.getValue() + ":" + desc.replaceAll("\\$\\{code\\}", code);
